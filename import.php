...
if( isset( $_FILES[ 'import_contacts' ] ) && isset( $_POST[ 'wpcrm_system_import_contacts_nonce' ] ) ){
	if( wp_verify_nonce( $_POST[ 'wpcrm_system_import_contacts_nonce' ], 'wpcrm-system-import-contacts-nonce' ) ){
		...
		if( !in_array( $file_type, $csv_types ) ){
			$errors[] = __( 'File not allowed, please use a CSV file.', 'wp-crm-system-import-contacts' );
		}
		if( $file_size > return_bytes( ini_get( 'upload_max_filesize' ) ) ){
			$errors[] = __( 'File size must be less than ', 'wp-crm-system-import-contacts' ) . ini_get( 'upload_max_filesize' );
		}				
		if( empty( $errors ) == true ){
					...
					if( wpcrm_system_import_contacts_duplicate( $title, $fileop ) ) {
						...
						if( $contactCategories != '' ) {
							...
							if ( is_wp_error( $contactTypes ) ) {
								$error[] = __( 'There was an error with the categories and they could not be set.', 'wp-crm-system-import-contacts' );
							}
						}
						if( $custom_fields ){
			...
			<div id="message" class="updated">
				<p><strong><?php _e( 'Contacts uploaded. ', 'wp-crm-system-import-contacts' ); echo $count_added; _e( ' added. ', 'wp-crm-system-import-contacts' ); echo $count_skipped; _e( ' skipped.', 'wp-crm-system-import-contacts' ); ?> </strong></p>
			</div>
		<?php } else { ?>
...
<div class="wrap">
	<div>
		<h2><?php _e( 'Import Contacts From CSV', 'wp-crm-system-import-contacts' ); ?></h2>
				...
				<form id="wpcrm_import_contacts" name="wpcrm_import_contacts" method="post" action="" enctype="multipart/form-data">
					<tr>
						...
						<td>
							...
							$link = sprintf( wp_kses( __( 'Important: Please make sure your CSV file is in the <a href="%s">correct format</a>. If it is out of order, your fields will not be imported correctly. ', 'wp-crm-system-import-contacts' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( $url ) );
							echo $link; ?>
						</td>
					</tr>
				</form>
				<form id="wpcrm_export_contacts" name="wpcrm_export_contacts" method="post" action="">
					<tr>
						<td>
							...
							<input type="submit" name="wpcrm_system_export_contacts" value="<?php _e('Export Contacts', 'wp-crm-system-import-contacts' ); ?>" />
						</td>
						<td></td>
					</tr>
				</form>
